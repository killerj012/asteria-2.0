...
public final class Session {
	...
	public void encode(WriteBuffer buffer) {
		...
		buffer.getBuffer().put(0,
				(byte) (buffer.getBuffer().array()[0] + encryptor.getKey()));
		...
		case LOGGING_IN:
			...
			if (DECODE_RSA) {
				if (!botLogin) {
					...
					int[] isaacSeed = { (int) (clientHalf >> 32),
			...
			for (int i = 0; i < player.getSkills().length; i++) {
				packetBuilder.sendSkill(i, player.getSkills()[i].getLevel(),
						player.getSkills()[i].getExperience());
			}
			...
			AssignWeaponInterface.assignInterface(player, player.getEquipment()
					.getContainer().getItem(Misc.EQUIPMENT_SLOT_WEAPON));
