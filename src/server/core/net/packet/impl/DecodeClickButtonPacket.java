...
public class DecodeClickButtonPacket extends PacketDecoder {
    ...
    public void decode(final Player player, PacketBuffer.ReadBuffer in) {
        ...
        case 13092:
            ...
            if (partner.getInventory().getContainer().freeSlots() < player
                    .getTradeSession().getOffering().size()) {
                player.getPacketBuilder()
                        .sendMessage(
                                partner.getUsername()
                                        + " does not have enough free slots for this many items.");
                return;
            }

            player.getTradeSession().setAcceptInitialOffer(true);
            player.getPacketBuilder().sendString("Waiting for other player...",
                    3431);
            partner.getPacketBuilder().sendString("Other player has accepted",
                    3431);

            if (player.getTradeSession().isAcceptInitialOffer()
                    && partner.getTradeSession().isAcceptInitialOffer()) {
        ...
        case 13218:
            ...
            if (player.getTradeSession().isAcceptConfirmOffer()
                    && partner.getTradeSession().isAcceptConfirmOffer()) {
        ...
        case 1097:
            if (player.isAutocast()) {
                ...
                if (player.getEquipment().getContainer()
                        .getIdBySlot(Misc.EQUIPMENT_SLOT_WEAPON) == 4675) {
        ...
        case 30108:
            if (player.getCombatSpecial() == null) {
                player.getPacketBuilder()
                        .sendMessage(
                                "This weapon does not have a special attack. Please contact developers ASAP.");
                return;
            }
            ...
            if (player.isSpecialActivated()) {
                ...
                if (player.getSpecialPercentage() < player.getCombatSpecial()
                        .getSpecialAmount()) {
                    player.getPacketBuilder().sendMessage(
                            "You do not have enough special energy!");
                    return;
                }
