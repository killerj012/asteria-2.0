...
public class DecodeNpcActionPacket extends PacketDecoder {
    ...
    public void decode(final Player player, ReadBuffer in) {
        ...
        case ATTACK_NPC:
            ...
            if (!NpcDefinition.getNpcDefinition()[attackMelee.getNpcId()]
                    .isAttackable()) {
                return;
            }
        ...
        case MAGE_NPC:
            ...
            if (!NpcDefinition.getNpcDefinition()[attackMagic.getNpcId()]
                    .isAttackable()) {
                return;
            }
            ...
            player.getMovementQueueListener().submit(new Runnable() {
                ...
                public void run() {
                    if (player.getPosition().withinDistance(
                            new Position(firstClickMob.getPosition().getX(),
                                    firstClickMob.getPosition().getY(),
                                    firstClickMob.getPosition().getZ()), 1)) {
        ...
        case SECOND_CLICK:
            ...
            player.getMovementQueueListener().submit(new Runnable() {
                ...
                public void run() {
                    if (player.getPosition().withinDistance(
                            new Position(secondClickNpc.getPosition().getX(),
                                    secondClickNpc.getPosition().getY(),
                                    secondClickNpc.getPosition().getZ()), 1)) {
