...
public abstract class PacketBuffer {
   ...
   /**
    * The default allocation size is the default amount of bytes allocated into
    * the buffer, if more bytes are needed the buffer is automatically expanded
    * by our {@link #allocate(int)} method.
    */
    private static final byte DEFAULT_ALLOCATION_SIZE = 16;
     ...
     * StreamgetBuffer(). BYTE_ACCESS is for reading/writing bytes, BIT_ACCESS is for
    ...
    /**
     * A backing byte buffer used to read and write data
     */
    private ByteBuffer buffer;

    /**
     * Constructs a new {@link PacketBuffer} with the specified byte buffer.
     * @param buffer The byte buffer, may not be {@code null}.
     * @throws NullPointerException If the specified buffer is {@code null}.
     */
    protected PacketBuffer(ByteBuffer buffer) {
    ...
    /** The current AccessType of the getBuffer(). */
     ...
     */
    public static ReadBuffer newReadBuffer() {
	return newReadBuffer(DEFAULT_ALLOCATION_SIZE);
    }
    
    public static ReadBuffer newReadBuffer(int size) {
	return newReadBuffer(ByteBuffer.allocate(size));
    }
    
    public static ReadBuffer newReadBuffer(ByteBuffer buffer) {
     ...
     */
    public static WriteBuffer newWriteBuffer() {
	return new WriteBuffer(ByteBuffer.allocate(DEFAULT_ALLOCATION_SIZE));
    }
    ...
    /**
     * Allocates a new {@link ByteBuffer} of the current buffers capacity plus the specified size if an overflow is detected.
     * @param size The amount of bytes to be written.
     * @return The new byte buffer instance.
     */
    abstract ByteBuffer allocate(int size);

    /**
     * Sets the AccessType of this StreamgetBuffer().
     * 
     * @param accessType
     *        the new AccessType
     */
    ...
    /**
     * Gets the AccessType of this StreamgetBuffer().
     * 
	...
	protected ReadBuffer(ByteBuffer buffer) {
	    super(buffer);
	}
	...
	protected ByteBuffer allocate(int size) {
	    throw new UnsupportedOperationException("incrementing size for read buffers is not supported.");
	}
	...
	protected WriteBuffer(ByteBuffer buffer) {
	    super(buffer);
	}
	 ...
	 */
	 ...
	 /**
	  * Writes the bytes from the argued buffer into this getBuffer(). This method
	  * does not modify the argued buffer, and please do not flip() the
	  * buffer before hand.
	  * 
	  * @param from
	  */
	 public WriteBuffer writeBytes(byte[] from) {
	     getBuffer().put(from);
	     return this;
	 }
	 
	 /**
	  * Writes the bytes into this getBuffer().
	  * 
	  * @param from
	  */
	 public WriteBuffer writeBytes(byte[] from, int size) {
	     getBuffer().put(from, 0, size);
	     return this;
	 }
	 ...
	 public WriteBuffer writeString(String string) {
	     setBuffer(allocate(string.getBytes().length + 1));
	...
	protected ByteBuffer allocate(int size) {
	    ...
	    if (buffer.position() + size > capacity) {
		ByteBuffer newBuffer = ByteBuffer.allocate(capacity + size);
		newBuffer.put(buffer.duplicate());
		System.out.println("Old size: " + buffer.capacity() + ", new size: " + newBuffer.capacity());
		return newBuffer;
	    }
