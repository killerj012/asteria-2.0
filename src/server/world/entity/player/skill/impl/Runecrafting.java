 ...
 * @author lare96
 */
public class Runecrafting extends SkillEvent {
     ...
     */
    public void teleport(final Player player, final Npc mob, final Position position) {
        ...
        Rs2Engine.getWorld().submit(new Worker(1, false) {
            ...
            public void fire() {
                 ...
                 * player/mob.
                 */
                mob.forceChat(Misc.randomElement(spell));
    ...
    public void craftRunes(Player player, Rune rune) {
        ...
        if (!player.getSkills()[Misc.RUNECRAFTING].reqLevel(rune.getLevelRequired())) {
            player.getPacketBuilder().sendMessage("You need a runecrafting level of " + rune.getLevelRequired() + " to craft " + rune.name().toLowerCase() + " runes.");
            fireResetEvent(player);
            return;
        }

        /** Checks if we even have rune essence. */
        if (!player.getInventory().getContainer().contains(RUNE_ESSENCE)) {
            player.getPacketBuilder().sendMessage("You need rune essence to craft " + rune.name().toLowerCase() + " runes.");
            fireResetEvent(player);
            return;
        }
        ...
        if (rune.getMultipleLevelsRequired()[0].getLevelRequired() != -1) {
             ...
             * one.
             */
            if (player.getSkills()[Misc.RUNECRAFTING].reqLevel(rune.getMultipleLevelsRequired()[0].getLevelRequired())) {
                ...
                for (int i = 0; i < multiples.length; i++) {
                    if (!player.getSkills()[Misc.RUNECRAFTING].reqLevel(multiples[i].getLevelRequired())) {
                ...
                int count = player.getInventory().getContainer().getCount(RUNE_ESSENCE);

                player.animation(new Animation(791));
                player.gfx(new Gfx(186));
                exp(player, rune.getExperience() * (count * multiples[0].getMultiply()));
                player.getInventory().deleteItem(new Item(RUNE_ESSENCE, count));
    ...
    public int eventFireIndex() {
        return SkillEvent.RUNECRAFTING;
    }

    @Override
    public SkillConstant skillConstant() {
        return SkillConstant.RUNECRAFTING;
    }
