 ...
 * @author lare96
 */
public class Thieving extends SkillEvent {

    // TODO: finish, check and revamp already...
    ...
    public enum TheftNpc {
         ...
         *        the possible items to receive when stealing.
         */
        TheftNpc(int requiredLevel, int experience, int stunDelay, Item... theftItem) {
     ...
     */
    public void stealNpc(Player player, TheftNpc theftMob, Npc mob) {
        if (!player.getSkills()[Misc.THIEVING].reqLevel(theftMob.getRequiredLevel())) {
            player.getPacketBuilder().sendMessage("You need a thieving level of " + theftMob.getRequiredLevel() + " to pickpocket this " + NpcDefinition.getNpcDefinition()[mob.getNpcId()].getName() + "");
            return;
        }
        ...
        if (player.getNpcTheftTimer().elapsed() > MOB_THIEVING_DELAY) {
            ...
            if (Misc.getRandom().nextInt(10) != 10) {
                ...
                if (player.getInventory().getContainer().hasRoomFor(stole)) {
                    player.animation(new Animation(881));
                    player.getInventory().addItem(stole);
                    exp(player, theftMob.getExperience());
                    player.getPacketBuilder().sendMessage("You successfully pickpocket the " + NpcDefinition.getNpcDefinition()[mob.getNpcId()].getName() + ".");
                } else {
                    player.getPacketBuilder().sendMessage("You should make room in your inventory before trying to pickpocket someone!");
                }
            } else {
    ...
    public void stealFromObject(Player player, TheftObject object) {
        if (!player.getSkills()[Misc.THIEVING].reqLevel(object.getRequiredLevel())) {
            player.getPacketBuilder().sendMessage("You need a thieving level of " + object.getRequiredLevel() + " to steal from this stall.");
            return;
        }
        ...
        if (player.getObjectTheftTimer().elapsed() > player.getLastTheftDelay()) {
            ...
            if (player.getInventory().getContainer().hasRoomFor(stole)) {
                ...
                player.getPacketBuilder().sendMessage("You successfully steal an item from the stall.");
            } else {
                player.getPacketBuilder().sendMessage("You should make room in your inventory before trying to pickpocket someone!");
            }
            ...
            player.getPacketBuilder().sendMessage("You should wait before trying to steal again or you'll look suspicious!");
        }
    }
    ...
    public int eventFireIndex() {
        return SkillEvent.THIEVING;
    }

    @Override
    public SkillConstant skillConstant() {
        return SkillConstant.THIEVING;
    }
}
