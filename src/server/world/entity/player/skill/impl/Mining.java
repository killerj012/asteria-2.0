...
public class Mining extends SkillEvent {
    ...
    public void mine(final Player player, final Ore ore, final Pickaxe pick, final Position rockPosition, final int objectId) {
        ...
        if (!player.getSkills()[Misc.MINING].reqLevel(ore.getLevel())) {
            player.getPacketBuilder().sendMessage("You need a mining level of " + ore.getLevel() + " to mine " + ore.name().toLowerCase().replaceAll("_", " ") + ".");
            return;
        }
        ...
        Rs2Engine.getWorld().submit(new Worker((Misc.getRandom().nextInt(getMiningTime(player, ore, pick)) + 1), false) {
            @Override
            public void fire() {
         ...
         */
        Rs2Engine.getWorld().submit(new Worker(4, true, WorkRate.APPROXIMATE_SECOND) {
            @Override
            public void fire() {
    ...
    public void prospect(Player player, Ore ore) {
        player.getPacketBuilder().sendMessage("This rock contains " + ore.name().toLowerCase().replaceAll("_", " ") + " ore.");
    }
    ...
    public Pickaxe getPickaxe(Player player) {
        for (Pickaxe pick : Pickaxe.values()) {
            ...
            if (player.getInventory().getContainer().contains(pick.getId()) || player.getEquipment().getContainer().contains(pick.getId())) {
                if (player.getSkills()[Misc.MINING].getLevel() >= pick.getLevel()) {
                    return pick;
                }

                player.getPacketBuilder().sendMessage("You are not a high enough level to use the " + pick.name().toLowerCase().replaceAll("_", " ") + " pickaxe.");
                return null;
            }
        }
