...
public abstract class CombatHitContainer {
    ...
    public static final CombatHitContainer NO_DAMAGE = new CombatHitContainer(
            null, null, true) {
        @Override
        public void onHit(Entity attacker, Entity victim, int totalDamage,
                boolean accurate) {
        }
    };
     ...
     *            if accuracy should be taken into account.
     */
    public CombatHitContainer(Hit[] hitSet, CombatType hitType,
        ...
        if (hitSet != null) {
            if (hitSet.length > 4) {
                ...
                throw new IllegalArgumentException(
                        "Illegal number of hits! The maximum number of hits per turn with magic is 1.");
            } else if (hitSet.length < 1) {
                throw new IllegalArgumentException(
                        "Illegal number of hits! The minimum number of hits per turn is 1.");
            }
     ...
     */
    public abstract void onHit(Entity attacker, Entity victim, int totalDamage,
            boolean accurate);
